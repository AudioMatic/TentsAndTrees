...
function B = solveTentjeBoompje(A,R,K)
 ...
 [rB,kB] = size(B) 
    ...
    for(j=2:kB-1)
       if B(1,j) == x then
            if B(2,j) <> x & B(1,j-1) <> x & B(1,j+1) <> x then
                B(1,j) = t
            end   
       end
    end
    ...
    for(j=2:kB-1)
       if B(rB,j) == x then
            if B(rB-1,j) <> x & B(rB,j-1) <> x & B(rB,j+1) <> x then
                B(rB,j) = t
            end   
       end
    end 
    ...
    for(i=2:rB-1)
       if B(i,1) == x then
           if B(i-1,1) <> x & B(i+1,1) <> x & B(i,2) <> x then
               B(i,1) = t
           end   
       end
    end
    ...
    for(i=2:rB-1)
       if B(i,rB) == x then
           if B(i-1,rB) <> x & B(i+1,rB) <> x & B(i,rB-1) <> x then
               B(i,rB) = t
           end   
       end
    end   
    ...
    // rechteronderhoek
           if B(rB,kB) == x then
            if B(rB-1,kB) <> x & B(rB,kB-1) <> x then
                B(rB,kB) = t
            end   
       end
